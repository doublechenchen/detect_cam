...
最新验证状态及未完成项见 [test-results/STATUS.md](test-results/STATUS.md)，拿起判定优化记录见 [test-results/PICKUP_OPTIMIZATION.md](test-results/PICKUP_OPTIMIZATION.md)。

当前 FP16 原生编译优化已在板上测得平均约 130ms、P95 164ms（136 个现场样本）；计时范围与限制见 [性能报告](test-results/PERFORMANCE.md)。
...
5. 持续观察到上抬且身份确认后，会显示对应 `SKU_info` 文本；显示框包含 track_id、SKU 和状态。短时丢失不绘制旧框，保留事件历史以等待恢复。包装部分遮挡时可在有界时间内继续关联已确认 SKU；身份不确定的帧不会独立确认拿起。
