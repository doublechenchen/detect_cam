...
public final class DetectEngine {
    public interface Listener { void result(List<Detection> boxes,int selectedSku,String message,long time); }
    ...
    private void process(byte[] frame,int w,int h,long time,int token) throws IOException {
        ...
        found=unique;
        List<Detection> shown=new ArrayList<>();int card=-1;String message;
        int sku=enrollment;
        if(sku>=0) {
            ...
            if(inside.size()==1){
                ...
                if(quality==null&&time-lastCapture>=650&&store.count(sku)<FeatureStore.TARGET){
                    lastCapture=time;float[] f=NativeRknn.embed(embed,frame,w,h,d.left,d.top,d.right,d.bottom);
            ...
            for(Detection d:found){Cached cached=null;
                ...
                if(cached==null){cached=new Cached();cached.feature=NativeRknn.embed(embed,frame,w,h,d.left,d.top,d.right,d.bottom);cached.match=store.match(cached.feature);cached.time=time;}
                ...
                if(m.sku>=0)observations.add(new ObjectTracker.Observation(d,f,m));
            }
            ...
            for(ObjectTracker.Track tr:tracks){
                ...
                states.append('#').append(tr.id).append(' ').append(name).append(' ').append(tr.stateText()).append("  ");
            }
        ...
        message+="\n检测 "+found.size()+" 件 · 分析 "+String.format(Locale.US,"%.1f",analysisFps)+" FPS / "+elapsed+" ms · 单目拿起推断";
        if(time-lastLog>=2000){lastLog=time;Log.i("DetectCam",message.replace('\n',' '));}
        publish(shown,card,message,time,token);
    }
    ...
    private void publish(List<Detection> boxes,int sku,String message,long time,int token){ui.post(()->{if(token==generation&&active)listener.result(boxes,sku,message,time);});}
